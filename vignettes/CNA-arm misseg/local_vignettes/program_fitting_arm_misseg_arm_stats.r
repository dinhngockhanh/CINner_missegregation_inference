...
#########
# load(paste0(model_name, "_sc_simulation_", 1, ".rda"))
...
for (stat in list_targets_library) {
  ...
  if (stat_target == "genome") {
    simulations_statistics[[stat_ID]] <- matrix(0, nrow = 1, ncol = 1)
  } else if (stat_target == "chromosome") {
    stat_chromosome_ID <- strsplit(strsplit(stat_details[grep("chromosome=", stat_details)], "=")[[1]][2], ",")[[1]]
    simulations_statistics[[stat_ID]] <- matrix(0, nrow = 1, ncol = length(stat_chromosome_ID))
  }
}
for (stat in list_targets_library) {
  stat_details <- strsplit(stat, ";")[[1]]
...
# list_targets[2, which(colnames(list_targets) %in% list_targets_misseg)] <- 1
